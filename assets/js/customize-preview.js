/* global openwebofficeBgColors, openwebofficePreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( openwebofficeBgColors ).forEach( function( context ) {
					openwebofficeGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function openwebofficeGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'openweboffice-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#openweboffice-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( openwebofficePreviewEls[ context ] ) {
				_.each( openwebofficePreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		openwebofficeGenerateColorA11yPreviewStyles( 'content' );
		openwebofficeGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
