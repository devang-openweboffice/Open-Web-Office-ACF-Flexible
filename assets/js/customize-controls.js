/* global openwebofficeBgColors, openwebofficeColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( openwebofficeBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( openwebofficeBgColors[ context ].color ) {
						backgroundColorValue = openwebofficeBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( openwebofficeBgColors[ context ].setting ).get();
					}
					openwebofficeSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( openwebofficeBgColors ).forEach( function( context ) {
			wp.customize( openwebofficeBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					openwebofficeSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		openwebofficeSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				openwebofficeSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function openwebofficeSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = openwebofficeColor( backgroundColor, accentHue );
	 ...
	 *
	 * @since Open Web Office 1.3
	 *
	 * @param {boolean} visible The visible value.
	 *
	 * @return {void}
	 */
	function openwebofficeSetRetineLogoVisibility( visible ) {
